...
describe MessagesController do
  ...
  context 'POST process_request' do
    context 'with valid_parameters' do
      ...
      let(:valid_attributes) {VALID_RESPONSE}
      let(:valid_parameters) {{message: valid_attributes}}
    
      before do 
        WebMock::stub_request(:post, "https://api:key-4tcw-c9e6mk4qk4c5br9so2odsksp-y3@api.mailgun.net/v2/barfoo.mailgun.org/messages").
         with(:body => {"from"=>"barfoodicus@barfoo.mailgun.org", "subject"=>"pry test", "text"=>"This is a standard response text", "to"=>"mfpiccolo@gmail.com"},
              :headers => {'Accept'=>'*/*', 'Content-Type'=>'application/x-www-form-urlencoded', 'User-Agent'=>'Faraday v0.8.7'}).
         to_return(:status => 200, :body => "", :headers => {})

        post :process_request, valid_parameters
      end
